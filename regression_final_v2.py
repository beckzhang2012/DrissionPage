...
def run_regression():
    ...
    try:
        ...
        passed = (result == 'failed' or result is None) and not part_file.exists()
        
        print_evidence("下载失败后.part清理", passed, states, part_file.exists(), final_file.exists(), 
                       0 if passed else 1, f"result={result}")
        results.append({
            'name': '下载失败后.part清理',
            'passed': passed,
            'state_transitions': states,
        ...
        print("等待任务完成（使用 timeout=30 秒）...")
        result, info = mission.wait(show=False, timeout=30)
