...
from ._funcs import (FileExistsSetter, PathSetter, BlockSizeSetter, 
                      set_charset, get_file_info, get_file_exists_mode,
                      verify_range_match, parse_content_range, INTEGRITY_ALGORITHMS)
...
class DownloadKit(object):
    ...
    def __init__(self, save_path=None, roads=10, driver=None,
                 file_exists='rename', session=None, goal_path=None):
        """
        ...
        self.save_path = goal_path or save_path or '.'
    ...
    def __call__(self, file_url, save_path=None, rename=None, suffix=None,
                 file_exists=None, show_msg=True, goal_path=None, 
                 integrity_algorithm: Optional[str] = None,
                 expected_hash: Optional[str] = None, **kwargs):
        """以阻塞的方式下载一个文件并返回结果
        :param file_url: 文件网址
        :param save_path: 保存路径
        :param rename: 重命名的文件名
        :param suffix: 指定后缀名
        :param file_exists: 遇到同名文件时的处理方式
        :param show_msg: 是否打印进度
        :param integrity_algorithm: 完整性校验算法 ('md5', 'sha256', None)
        :param expected_hash: 期望的哈希值
        :param kwargs: 连接参数
        :return: 任务结果和信息组成的tuple
        """
        save_path = goal_path or save_path
        return self.download(file_url=file_url, save_path=save_path, rename=rename, suffix=suffix,
                             file_exists=file_exists, show_msg=show_msg,
                             integrity_algorithm=integrity_algorithm,
                             expected_hash=expected_hash, **kwargs)
    ...
    def add(self, file_url, save_path=None, rename=None, suffix=None,
            file_exists=None, split=None, goal_path=None,
            integrity_algorithm: Optional[str] = None,
            expected_hash: Optional[str] = None, **kwargs):
        """添加一个下载任务并将其返回
        :param file_url: 文件网址
        ...
        :return: 任务对象
        """
        if integrity_algorithm is not None and integrity_algorithm not in INTEGRITY_ALGORITHMS:
            raise ValueError(f"不支持的完整性校验算法: {integrity_algorithm}，支持: {INTEGRITY_ALGORITHMS}")
        
        if integrity_algorithm and expected_hash is None:
            raise ValueError("指定了完整性校验算法但未提供期望的哈希值")
        ...
        mission = Mission(self._missions_num, self, file_url, str(save_path or self.save_path), rename, suffix,
                          ...
                          kwargs, integrity_algorithm=integrity_algorithm, expected_hash=expected_hash)
    ...
    def download(self, file_url, save_path=None, rename=None, suffix=None,
        ...
        if show_msg:
            ...
            self._print_mode = None
        save_path = goal_path or save_path
        r = self.add(file_url=file_url, save_path=save_path, rename=rename, suffix=suffix, 
                     file_exists=file_exists, split=False,
                     integrity_algorithm=integrity_algorithm, expected_hash=expected_hash,
    ...
    def get_failed_missions(self):
        """返回失败任务列表"""
        return [i for i in self._missions.values() if i.result is False or i.result == 'failed']
    ...
    def _when_mission_done(self, mission):
        ...
        """
        self._running_count -= 1
        if self._print_mode == 'all' or (self._print_mode == 'failed' and mission.result in (False, 'failed', 'integrity_failed')):
            print(f'[{mission.RESULT_TEXTS[mission.result]}] {mission.data.url} {mission.info}')

        if self._log_mode == 'all' or (self._log_mode == 'failed' and mission.result in (False, 'failed', 'integrity_failed')):
            data = ('下载结果',
                    mission.data.url,
    ...
    def _download(self, mission_or_task, thread_id):
        ...
        if isinstance(mission_or_task, Task):
            ...
            requested_start = task.range[0]
            requested_end = task.range[1]
            kwargs['headers']['Range'] = f"bytes={requested_start}-{requested_end}"
            
            task.mission._record_state('request_range', 
                                       f'请求 Range: bytes={requested_start}-{requested_end}')
            ...
            if r:
                ...
                if content_range:
                    ...
                    if not match_ok:
                        task.set_range_mismatch(match_reason)
                        task.mission._record_state('range_mismatch_handled', match_reason)
                        
                        if '206' not in str(r.status_code):
                            task.mission._record_state('fallback_full', '服务端不支持Range，回退到完整下载')
                            task._set_done('success', '服务端不支持Range，完整下载')
                            return
                        
                        task._set_done('failed', match_reason)
                        return
                
                accept_ranges = r.headers.get('Accept-Ranges', '')
                if accept_ranges != 'bytes' and r.status_code != 206:
                    task.mission._record_state('no_range_support', 
                                               f'Accept-Ranges: {accept_ranges}，状态码: {r.status_code}')
                
                _do_download(r, task, False)
            else:
                task._set_done('failed', inf)
        ...
        if not r:
            mission._break_mission('failed', inf)
            return
        ...
        if file_exists == 'add' and full_Path.exists():
            ...
            mission._record_state('resume', f'断点续传，已存在 {mission.data.offset} 字节')

        first = False
        accept_ranges = r.headers.get('Accept-Ranges', '')
        if split and file_size and file_size > self.block_size and accept_ranges == 'bytes':
            first = True
            mission._record_state('split_download', f'分块下载，每块 {self.block_size} 字节')
            ...
            if accept_ranges != 'bytes':
                mission._record_state('no_range_support', f'服务端不支持Range: {accept_ranges}')
...
def _do_download(r: Response, task: Task, first: bool = False):
    ...
    try:
        ...
        result, info = 'failed', f'下载失败。{r.status_code} {e}'
