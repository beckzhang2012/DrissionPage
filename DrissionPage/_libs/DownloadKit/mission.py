...
from ._funcs import copy_session, set_session_cookies, verify_file_integrity, get_part_file_path


class MissionData(object):
    def __init__(self, url, save_path, rename, suffix, file_exists, split, kwargs, offset=0,
                 integrity_algorithm: Optional[str] = None, expected_hash: Optional[str] = None):
        """保存任务数据的对象
        :param url: 下载文件url
...
class BaseTask(object):
    _DONE = 'done'
    RESULT_TEXTS = {'success': '成功', 'skipped': '跳过', 'canceled': '取消', 
                    'failed': '失败', 'integrity_failed': '校验失败',
                    False: '失败', None: '未知'}
    ...
    def set_states(self, result=None, info=None, state='done'):
        """设置任务结果值（原子操作）
        :param result: 结果：'success'、'skipped'、'canceled'、'failed'、'integrity_failed'、False、None
        :param info: 任务信息
...
class Mission(BaseTask):
    def __init__(self, ID, download_kit, file_url, save_path, rename, suffix, 
                 file_exists, split, encoding, kwargs,
        ...
        self._data = MissionData(file_url, save_path, rename, suffix, 
        ...
        self._task_done_lock = Lock()
        self._state_transitions = []
        self._record_state('waiting', '初始化')
    ...
    def _record_state(self, state: str, info: str):
        """记录状态流转
        :param state: 状态
        :param info: 附加信息
        """
    ...
    def cancel(self) -> None:
        """取消该任务，停止未下载完的task"""
        self._record_state('cancel', '用户取消')
    ...
    def wait(self, show=True, timeout=0):
            ...
            if show and self.size:
                try:
                    if self.part_path and self.part_path.exists():
                        ...
                        current_size = 0
                    rate = round((current_size / self.size) * 100, 2)
                    print(f'\r{rate}% ', end='')
        ...
        if show:
            print()
            print(f'状态流转: {[t["state"] for t in self._state_transitions]}')
            if self.result is False or self.result == 'failed':
                print(f'下载失败 {self.info}')
            elif self.result == 'integrity_failed':
                print(f'完整性校验失败 {self.info}')
    ...
    def _set_path(self, path):
        ...
        self._path = path
        self._part_path = get_part_file_path(path)
        self.recorder.set.path(self._part_path)
    ...
    def _a_task_done(self, is_success, info):
        ...
        if is_success is False or is_success == 'failed':
            self._break_mission('failed', info)
            return

        with self._task_done_lock:
...
class Task(BaseTask):
    ...
    @property
    def range_mismatch(self):
        """返回是否发生 Range 不匹配"""
        return self._range_mismatch
    ...
    def set_range_mismatch(self, reason: str):
        """标记 Range 不匹配
        :param reason: 不匹配原因
        """
        self._range_mismatch = True
        self.mission._record_state('range_mismatch', reason)

    def _set_done(self, result, info):
        """设置一个子任务为done状态
        :param result: 结果：'success'、'skipped'、'canceled'、'failed'、False、None
        :param info: 任务信息
        :return: None
        """
        self.set_states(result=result, info=info, state=self._DONE)
